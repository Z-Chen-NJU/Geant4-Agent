...
def _generate_one(structure: str, config: str, seed: int, max_retries: int) -> Dict[str, Any]:
    rng = random.Random(seed)
    prompt = PROMPT_TEMPLATE.format(structure=structure)
    for _ in range(max_retries):
        resp = chat(prompt, config_path=config, temperature=0.2)
...
def main() -> None:
    ...
    parser.add_argument("--config", default="bert_lab/ollama_config.json")
    ...
    for i in range(args.n):
        ...
        obj = _generate_one(structure, args.config, seed, args.max_retries)
        samples.append(obj)
